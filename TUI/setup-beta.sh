...
# Use sudo with the entered password
        ...
        if [ $? -eq 0 ]; then
            ...
            driverCheck=$(lsmod | grep nvidia)
            if [ $? -ne 0 ]; then
                read -p "NVIDIA drivers are not installed. Do you want to install them? (y/n): " response
                ...
                sudo dnf install akmod-nvidia nvidia-settings nvidia-modprobe nvtop nv-codec-headers -y
                else
